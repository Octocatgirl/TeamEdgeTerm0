...
let animals = ["Hamster", "squirel", "rat", "pig", "cat", "dog", "mouse", "lizard", "horse", "monkey"]
...
let friends = ["julian", "fatima", "chris", "watson", "mikah", "caleb"]

//-->TODO Prompt the user to "Guess" if an element is present. Store their response in a varaible
let guess = READLINE.question('What do you think one of my friends names are? >>').toLowerCase

function Myfriend() {
    if (friends.includes(guess)) {
        console.log("That is one of my friends name. CONGRATS!")
    } else {
        console.log("That is not my friend.")
    }
}
...
//this is how you can nest for loops, one insde the other! These loops through all the colors, and then through all the characters in the color
for (let i = 0; i < colors.length; i++) {
    //for all the colors:
...
let sentence = READLINE.question('Give me a sentence >> ')
let word = sentence.split(" ")

for (let i = 0; i < word.length; i++){
    console.log(word[i])
    for(let j = 0; j < word[i].length; i++){
        console.log(" "+ wordd[i][j])
    }
}
